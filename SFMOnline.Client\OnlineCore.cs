...
private async Task<(bool ok, string msg, int code, string token)> ConnectRelayFlow(string host, int port, string pwd)
{
    ...
    // 端口回退：主端口（如 7000）被部分网络 QoS/封锁时，自动尝试标准风格备用端口
    var portCandidates = new List<int> { port };
    if (port != 8443) portCandidates.Add(8443);
    if (port != 8090) portCandidates.Add(8090);
    int successPort = port;
    foreach (var tryPort in portCandidates)
    {
        for (int i = 1; i <= 3 && !connected; i++)
        {
            if (i > 1 || tryPort != port) Toast("正在连接 " + host + ":" + tryPort + "（" + i + "/3）...");
            connected = RelayTcp.Connect(host, tryPort, out tcpIdentity);
            if (!connected && i < 3) await Task.Delay(4000);
        }
        if (connected) { successPort = tryPort; break; }
    }
    if (!connected)
    {
        string err = RelayTcp.LastError.Length > 0 ? "（" + RelayTcp.LastError + "）" : "";
        return (false, "连接服务器超时，已尝试端口 " + string.Join("/", portCandidates) + err + "。请稍后重试或更换网络", -100, "");
    }
    if (successPort != port) Toast("已通过备用端口 " + successPort + " 连接成功");
