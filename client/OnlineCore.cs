...
namespace SFMOnline
{
    ...
    [HideFromIl2Cpp]
    private string PeerId => _relayMode ? _relayPlayerId : (IsHosting ? "host" : (_client != null ? _client.PeerId : ""));
        [HideFromIl2Cpp]
...
private void SendRelayBones()
{
    ...
    if (Time.unscaledTime - _lastRelayBoneAt < 0.10f) return;
...
private void SendRelayMotion()
{
    ...
    int hz = Math.Max(10, Math.Min(30, Settings.SyncRateHz.Value));
    float now = Time.unscaledTime;
    if (!stateChanged && now - _lastRelayMotionSend < 1f / hz) return;
...
private void DrawAuthGate()
{
    ...
    if (_authMode == "login" || _authMode == "register")
    {
        ...
        if (!_authUseCode)
        {
            ...
            _authPass = UiTextField("auth_pass", new Rect(_uiX + 64f, _uiY, 140f, h), _authPass, true, out _);
            _uiY += step;
            if (_authMode == "register")
            {
                if (_canLabel) GUI.Label(new Rect(_uiX, _uiY, 60f, h), Lang.Get("auth_pass2"));
                _authPass2 = UiTextField("auth_pass2", new Rect(_uiX + 64f, _uiY, 140f, h), _authPass2, true, out _);
                _uiY += step;
            }
        }
        if (_authMode == "register" && _canLabel)
        {
            GUI.Label(new Rect(_uiX, _uiY, _uiW, h * 3), Lang.Get("auth_register_hint"), WrapStyle());
            _uiY += step * 3;
        }
...
private void DrawMap(float x, float y, float size)
{
    ...
    if (Time.unscaledTime - _mapCacheAt > 5f)
    {
        ...
        try
        {
            ...
            if (cols != null)
                for (int i = 0; i < cols.Length && _mapObstaclePts.Count < 120; i++)
                {
                    var col = cols[i];
                    if (col == null || col.isTrigger || !col.enabled) continue;
                    if (col.bounds.size.magnitude > 60f) continue;
    ...
    foreach (var p in _mapPortalPts) if (Mathf.Abs(p.y - selfPos.y) < 4f) _mapFitPts.Add(p);
    foreach (var p in _mapRoutePts) if (Mathf.Abs(p.y - selfPos.y) < 4f) _mapFitPts.Add(p);
    foreach (var p in _mapDoorPts) if (Mathf.Abs(p.y - selfPos.y) < 4f) _mapFitPts.Add(p);
    foreach (var p in npcPts) if (Mathf.Abs(p.y - selfPos.y) < 4f) _mapFitPts.Add(p);
    foreach (var kv in _relayPositions)
    ...
    foreach (var p in _mapObstaclePts)
        if (Mathf.Abs(p.y - selfPos.y) < 4f)
        {
            Vector2 opos = Project(p);
            if (!mapRect.Contains(opos)) continue;
            GUI.color = new Color(0.55f, 0.55f, 0.58f, 0.75f);
            GUI.DrawTexture(new Rect(opos.x - 2f, opos.y - 2f, 4f, 4f), WhiteTex());
        }
    foreach (var p in _mapRoutePts)
        if (Mathf.Abs(p.y - selfPos.y) < 4f) CircleMarker(p, new Color(0.20f, 0.82f, 0.94f, 0.95f), 2.5f, "");
    foreach (var p in _mapDoorPts)
        if (Mathf.Abs(p.y - selfPos.y) < 4f) CircleMarker(p, new Color(1f, 0.55f, 0.12f, 1f), 4f, "门");
    foreach (var p in _mapPortalPts)
        if (Mathf.Abs(p.y - selfPos.y) < 4f) CircleMarker(p, new Color(0.72f, 0.34f, 1f, 1f), 4f, "");
    foreach (var p in npcPts)
        if (Mathf.Abs(p.y - selfPos.y) < 4f) CircleMarker(p, new Color(1f, 0.82f, 0.12f, 1f), 3.5f, "");
...
private void DrawAboutPanel()
{
    ...
    if (_canButton && SButton(new Rect(_uiX, _uiY, _uiW, h), "🔗 SFM Online 开服源码：GitHub"))
        OpenExternalUrl("https://github.com/b9348?tab=repositories&q=SFMOnline");
    _uiY += step;
    if (_canButton && SButton(new Rect(_uiX, _uiY, _uiW, h), "🔗 其他项目：https://github.com/b9348/sfmmm/releases"))
        OpenExternalUrl("https://github.com/b9348/sfmmm/releases");
    _uiY += step;
...
internal void OnGUI()
{
    ...
    {
        if (Event.current.shift) ForceDumpDiagnostics();
        else if (!_loggedIn)
        {
            _onlineMenuOnly = false;
            _showMenu = true;
        ...
        private sealed class GhostPlayer
        {
            ...
            private static bool IsSuspiciousAddonRenderer(Renderer r)
            {
                try
                {
                    ...
                    float min = Mathf.Max(0.01f, Mathf.Min(s.x, Mathf.Min(s.y, s.z)));
                    // 只隐藏真正的巨型异常块（缺资源生成的大白条），正常服装/身体网格不再误杀。
                    if (max > 8f || (max > 2.5f && max / min > 40f)) return true;
