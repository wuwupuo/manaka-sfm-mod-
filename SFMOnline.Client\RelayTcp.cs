    ...
    internal static class RelayTcp
    {
        ...
        public static bool Connect(string host, int port, out string peerIp)
        {
            ...
            try
            {
                ...
                // 国际线路（大陆→香港等）晚高峰握手可能需 10-20 秒，超时给足避免误报"端口不可达"
                int connectTimeoutSec = 20;
                try { connectTimeoutSec = Math.Max(10, Math.Min(30, Settings.ConnectTimeoutSec.Value)); } catch { }
                if (!connectTask.Wait(TimeSpan.FromSeconds(connectTimeoutSec)) || !c.Tcp.Connected)
                    throw new TimeoutException("连接超时（" + connectTimeoutSec + "秒）");
