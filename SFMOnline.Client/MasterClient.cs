...
namespace SFMOnline
{
    ...
    {
        public static string DefaultDomain => "wuwupuo1.xtxt.xyz";
        public static string StableUpdateUrl => "https://wuwupuo1.xtxt.xyz/public_html/sfm_api/update.php";
        private static readonly string[] CandidatePaths =
        {
            "/public_html/sfm_api/api.php",   // /public_html/sfm_api/api.php
            "/public_html/sfm_api/all.php",  // /public_html/sfm_api/all.php
            "/public_html/sfm_api/master.php", // /public_html/sfm_api/master.php
            "/sfm_api/master.php",               // /sfm_api/master.php
            "/public_html/master.php",           // /public_html/master.php
        ...
        public static async Task<bool> Download(string url, string destPath)
        {
            ...
            try
            {
                using (var resp = await Http.GetAsync(url))
                {
                    ...
                    // 签名校验：同时下载 .sig 签名文件，验证 DLL 完整性，防篡改/供应链攻击
                    ...
                    try
                    {
                        using (var sigResp = await Http.GetAsync(sigUrl))
                        {
                            ...
                            {
                                ClientLog.Write("签名文件缺失，拒绝更新: " + sigUrl + " HTTP=" + (int)sigResp.StatusCode);
                                return false;
                            }
                            string sigB64 = (await sigResp.Content.ReadAsStringAsync()).Trim();
                            if (!VerifySignedDll(bytes, sigB64))
                            {
                                ClientLog.Write("更新包签名验证失败，拒绝替换本地插件（可能被篡改）");
                                return false;
                            }
                        }
                    }
                    ...
                    {
                        ClientLog.Write("签名下载异常，拒绝更新: " + ex.Message);
                        return false;
                    }
        ...
        // 内置 RSA 公钥（仅用于验证官方签名，私钥只保存在服务器）
        private const string UpdatePublicKeyXml =
            "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";

        private static bool VerifySignedDll(byte[] dllBytes, string sigBase64)
        {
            try
            {
                ...
                using (var rsa = new RSACryptoServiceProvider())
                {
                    ...
                    using (var sha = System.Security.Cryptography.SHA256.Create())
                        hash = sha.ComputeHash(dllBytes);
                    return rsa.VerifyHash(hash, System.Security.Cryptography.CryptoConfig.MapNameToOID("SHA256"), signature);
                }
            }
            catch { return false; }
        }
